...
(def ^:private validator-cache (atom {:registry-snapshot nil :cache {}}))

(defn- -cached-validator [schema]
  (let [current-registry @gr.reg/schema-atom]
    (or
     (let [{:keys [registry-snapshot cache]} @validator-cache]
       (when (identical? current-registry registry-snapshot)
         (get cache schema)))
     (let [v (m/validator schema {:registry gr.reg/registry})]
       (swap! validator-cache
              (fn [{:keys [registry-snapshot] :as state}]
                (if (identical? current-registry registry-snapshot)
                  (assoc-in state [:cache schema] v)
                  {:registry-snapshot current-registry :cache {schema v}})))
       v))))

(defn validate [schema value] ((-cached-validator schema) value))
