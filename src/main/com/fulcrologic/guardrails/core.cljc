...
#?(:clj
   (defn- process-defn-tail
     ...
     (let [{:keys                                                      [env fn-name]
           ...
           opts-args-sym   (gensym "__gr_opts_args")
           opts-ret-sym    (gensym "__gr_opts_ret")
           ...
           args-check      `(when ~argspec
                              (when-not (gr.cfg/-excluded? ~(first exclusion-coord) ~(second exclusion-coord))
                                (run-check ~opts-args-sym ~argspec ~arg-syms)))
           ...
           ret-check       `(when (and ~retspec
                              ...
                              (run-check ~opts-ret-sym ~retspec ~ret))
           ...
           check-body      `(let [{~argspec :args ~retspec :ret} ~fspec
                              ...
                              (let [~f ~real-function
                                ...
                                (when-not throttle#
                                  ~ret-check)
                                ~ret))]
       {:bindings [opts-args-sym (assoc opts :args? true)
                   opts-ret-sym (assoc opts :args? false)]
        :tail     `(~@(remove nil? [raw-arg-vec prepost])
                    ~(if trace?
...
#?(:clj
   (defn- generate-defn
     ...
     (let [conformed-gdefn    (s/conform ::>defn-args forms)
           ...
           processed-results  (let [process-cfg      {:env     env
                                                      :config  config
                                                      :fn-name fn-name}
                                    fn-tail-or-tails (val conformed-fn-tails)]
                                (case arity
                                  :arity-1 [(process-defn-tail process-cfg fn-tail-or-tails)]
                                  :arity-n (mapv (partial process-defn-tail process-cfg) fn-tail-or-tails)))
           opts-bindings      (vec (mapcat :bindings processed-results))
           processed-fn-tails (map :tail processed-results)
           ...
           throttle-decls     (if add-throttling?
                                ...
                                [])
           all-decls          (into throttle-decls opts-bindings)]
       `(let ~all-decls
