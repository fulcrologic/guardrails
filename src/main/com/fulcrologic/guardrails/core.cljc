...
#?(:clj (try
          (do
            (ns-unalias (find-ns 'com.fulcrologic.guardrails.core) 'ana-api)
...
#?(:cljs (do
           (defonce devtools-error-formatter
...
(s/def ::guardrails-config
  (s/and (s/keys :req [::defn-macro
                       ::expound])))

(s/assert ::guardrails-config cfg/default-config)
